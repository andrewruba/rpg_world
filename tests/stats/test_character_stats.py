...
def arcanist():
    ...
    return Character(name="Arcanist", stats=stats)
...
def test_apply_zero_health_effect(arcanist):
    ...
    Test applying a focus increase effect to the character.
    """
    health_effect = Effect(attribute='health', formula=SimpleChangeFormula(0))
    health_effect.apply(arcanist)
...
def test_apply_health_effect_above_max(arcanist):
    ...
    Test applying a focus increase effect to the character.
    """
    health_effect = Effect(attribute='health', formula=SimpleChangeFormula(1000))
    health_effect.apply(arcanist)
...
def test_character_alive_status(arcanist):
    ...
    """
    health_effect = Effect(attribute='health', formula=SimpleChangeFormula(-120))
    health_effect.apply(arcanist)
